...
describe("Mutex with using declarations", () => {
	let mutex: Mutex<number>;
	...
	describe("Basic using Operations", () => {
		it("should automatically release lock when using scope ends", async () => {
			{
				using guard = await mutex.lock();
				expect(guard.value).toBe(0);
			} // guard is automatically released here

			// Should be able to acquire lock immediately after scope ends
			const newGuard = mutex.tryLock();
			expect(newGuard).not.toBeNull();
			newGuard?.release();
		});

		it("should handle sequential using declarations", async () => {
			const operations: string[] = [];

			{
				using guard1 = await mutex.lock();
				operations.push("acquired guard1");
				guard1.value = 1;
			} // guard1 released
			operations.push("after guard1");

			{
				using guard2 = await mutex.lock();
				operations.push("acquired guard2");
				guard2.value = 2;
			} // guard2 released
			operations.push("after guard2");
			...
			using finalGuard = await mutex.lock();
			expect(finalGuard.value).toBe(2);
		});
	...
	describe("Value Operations with using", () => {
		it("should maintain value consistency across using blocks", async () => {
			{
				using guard = await mutex.lock();
				guard.value = 1;
			}
		...
		it("should handle concurrent operations with using", async () => {
			const operations = 5;
...
describe("MutexCollection with using declarations", () => {
	let collection: MutexCollection<string, number>;
	...
	it("should handle multiple keys with using", async () => {
		const operations: string[] = [];

		{
			using guard1 = await collection.lock("key1");
			operations.push("acquired key1");
			guard1.value = 1;
		}
		operations.push("released key1");

		{
			using guard2 = await collection.lock("key2");
			operations.push("acquired key2");
			guard2.value = 2;
		}
		operations.push("released key2");
	...
	it("should handle concurrent operations on different keys", async () => {
		...
		const promises = keys.map(async (key) => {
			using guard = await collection.lock(key);
			guard.value = 1;
		});
	...
	it("should maintain order within same key", async () => {
		const results: number[] = [];
		const key = "test";
	...
	it("should handle errors in using blocks", async () => {
		expect.assertions(2);

		try {
			using guard = await collection.lock("error-key");
			throw new Error("Test error");
		} catch (error) {
			expect(error).toBeInstanceOf(Error);

			// Should be able to acquire lock after error
			using newGuard = await collection.lock("error-key");
			expect(newGuard.value).toBe(0);
		}
	});
});
